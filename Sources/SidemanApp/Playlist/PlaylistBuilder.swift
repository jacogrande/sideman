...
actor PlaylistBuilder {
    ...
    func buildPlaylist(
        ...
        // Over-fetch to account for join canonicalization, deduplication,
        // and fuzzy matching attrition before final truncation.
        // Co-credit joins have higher attrition so use a larger multiplier.
        let overfetchMultiplier = request.mode == .coCredit ? 3.0 : 2.0
        let fetchCount = min(Int(Double(request.maxTracks) * overfetchMultiplier), discography.recordings.count)
    ...
    private func fallbackArtistQueries(for request: PlaylistBuildRequest) -> [String] {
        if request.mode == .coCredit, let coCredit = request.coCredit {
            return dedupeArtists([coCredit.artistA.name, coCredit.artistB.name])
        }
        return dedupeArtists([request.artistName])
    }

    private func dedupeArtists(_ values: [String]) -> [String] {
        var seen = Set<String>()
        var ordered: [String] = []
        for value in values {
